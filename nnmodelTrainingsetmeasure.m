...
lambda=0;
J = nnCostFunction(initial_nn_params,length(initial_nn_params_1),input_layer_size,hidden_layer_size_1,hidden_layer_size_2,output_layer_size, Xaug,Y, lambda);
...
lambda=0;
costFunction =@(p)nnCostFunction(p,length(initial_nn_params_1),input_layer_size,hidden_layer_size_1,hidden_layer_size_2,output_layer_size, Xaug,Y, lambda);
%[nn_params, cost] = fmincg(costFunction, initial_nn_params, options);
list=0;
[nn_params, cost,list] = fmincg(costFunction, initial_nn_params, options);
...
[predict1, predict2]=forwardpropagation(Xaug,Y,nn_params,length(initial_nn_params_1),input_layer_size,hidden_layer_size_1,hidden_layer_size_2,output_layer_size);
predict1=predict1.*nfY(:,1);
predict2=predict2.*nfY(:,2);
validY=Y.*nfY;
result=[predict1 validY(:,1) predict2 validY(:,2)];
disp(result(1:200,:));
fprintf("Press Enter\n");
pause();

score=sum(abs(([predict1 predict2]-validY))< [std2(validY(:,1)) 5])./m
plot(list);